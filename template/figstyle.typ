...
// 全书图形统一样式（figstyle）
// ------------------------------------------------------------
// SYNC: 本文件与 typst-note-author/template/figstyle.typ 的代码相同，仅两处
// 差异：import 的配色字典名（这边 book-colors，那边 note-colors）与个别注释
// 措辞（书/笔记的叫法）。改这里要同步改那边，否则两套模板的图形样式会悄悄
// 漂移；typst-note-author/scripts/check_sync.py 可机检。
// ------------------------------------------------------------
...
// ---- 调色板：取自本书主题红与提示框配色，保证插图与版式同源 ----
// （色值集中在 colors.typ，此处只是别名）
#let palette = book-colors

// 多系列数据的取色顺序：同一章内按序取用，前后一致
